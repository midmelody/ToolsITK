...
void printStat(int iter, float sstvd, float vedcoeff, float smooth, float cost_sstvd, float cost_ved, float cost_smooth, float f, float cost_lmk, float JacMin, float JacMax, FILE* fp, FILE* fpStat, int& res_iter)
{
    float JacMaxInv=1.0/JacMax;
    printf("%3d  %8.2f  %8.2f (%10.2f)  %8.2f (%10.2f)  %10.2f  %8.2f  %8.2f  %8.2f  %8.2f\n",iter, sstvd*cost_sstvd, vedcoeff*cost_ved, cost_ved, smooth*cost_smooth, cost_smooth, f, cost_lmk, JacMin, JacMaxInv, vedcoeff);
    fprintf(fp, "%3d  %8.2f  %8.2f (%10.2f)  %8.2f (%10.2f)  %10.2f  %8.2f  %8.2f  %8.2f  %8.2f\n",iter, sstvd*cost_sstvd, vedcoeff*cost_ved, cost_ved, smooth*cost_smooth, cost_smooth, f, cost_lmk, JacMin, JacMaxInv, vedcoeff);
    fprintf(fpStat, "%3d  %8.2f  %18.2f  %18.2f  %16.2f  %8.2f  %8.2f  %8.2f  %8.2f\n",res_iter, sstvd*cost_sstvd, vedcoeff*cost_ved, smooth*cost_smooth, f, cost_lmk, JacMin, JacMaxInv, vedcoeff);
}
/*************************************************************************
...
void lbfgsbminimize(const int& n,
     ...
     const ap::real_1d_array& l,
     const ap::real_1d_array& u,
     int& info, float* arrayBasis0, float* arrayBasis1, float* arrayBasis2, short int* arrayLocation, int xdim, int ydim, int zdim, int num_element_x, int num_element_y, int num_element_z, int grid_space_x, int grid_space_y, int grid_space_z, float* imagein1, float* imagein2, float* grad_x_T, float* grad_y_T, float* grad_z_T, float* c_x, float* c_y, float* c_z, float* u_x_existing, float* u_y_existing, float* u_z_existing, float* u_x_total, float* u_y_total, float* u_z_total, unsigned char* mask, int maskCT2, int num_lmk, int* xc_1, int* yc_1, int* zc_1, int* xc_2, int* yc_2, int* zc_2, float spacing_x, float spacing_y, float spacing_z, float sstvd, float param, float smooth, float alpha, float beta, float gamma, FILE* fp, float* vm1, float* vm2, float* vm_grad_x_T, float* vm_grad_y_T, float* vm_grad_z_T, float ved, float* precalcu, float* precalcux, float* precalcuy, float* precalcuz, float* precalcu2x, float* precalcu2y, float* precalcu2z, FILE* fpStat, int& res_iter, float& cost_sstvd, float& cost_ved, float& cost_smooth, float& cost_lmk, float& JacMin, float& JacMax)
{
    ...
    funcgrad(x, f, g, arrayBasis0, arrayBasis1, arrayBasis2, arrayLocation, xdim, ydim, zdim, num_element_x, num_element_y, num_element_z, grid_space_x, grid_space_y, grid_space_z, imagein1, imagein2, grad_x_T, grad_y_T, grad_z_T, c_x, c_y, c_z, u_x_existing, u_y_existing, u_z_existing, u_x_total, u_y_total, u_z_total, mask, maskCT2, num_lmk, xc_1, yc_1, zc_1, xc_2, yc_2, zc_2, spacing_x, spacing_y, spacing_z, sstvd, param, smooth, alpha, beta, gamma, cost_sstvd, cost_lmk, cost_smooth, vm1, vm2, vm_grad_x_T, vm_grad_y_T, vm_grad_z_T, ved, cost_ved, vedcoeff, precalcu, precalcux, precalcuy, precalcuz, precalcu2x, precalcu2y, precalcu2z, JacMin, JacMax);
    printStat(0, sstvd, vedcoeff, smooth, cost_sstvd, cost_ved, cost_smooth, f, cost_lmk, JacMin, JacMax, fp, fpStat, res_iter);
    ...
    while(true)
    {
        ...
        iter = iter+1;
        res_iter++;
        printStat(iter, sstvd, vedcoeff, smooth, cost_sstvd, cost_ved, cost_smooth, f, cost_lmk, JacMin, JacMax, fp, fpStat, res_iter);
        ...
        //ddum = ap::minreal(fabs(fold), ap::minreal(fabs(f), double(1)));
        ddum = double(1);
        if( fold-f<=epsf*ddum )
        {
            info = 1;
