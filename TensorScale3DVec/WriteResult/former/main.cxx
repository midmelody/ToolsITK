...
int main(int argc, char *argv[])
{
  ...
  Ori1XReader->SetFileName( "temp/GradientDTXSmooth.img" );
  Ori1XReader->Update();
  ImageReaderType::Pointer Ori1YReader = ImageReaderType::New();
  Ori1YReader->SetFileName( "temp/GradientDTYSmooth.img" );
  Ori1YReader->Update();
  ImageReaderType::Pointer Ori1ZReader = ImageReaderType::New();
  Ori1ZReader->SetFileName( "temp/GradientDTZSmooth.img" );
  Ori1ZReader->Update();
  ...
  for(i=0;i<pRow;i++)
      ...
      for(k=0;k<pSli;k++)
	{
	  ...
	  pixelIndex[1]=j;
	  pixelIndex[2]=k;
	  Size1Image->SetPixel(pixelIndex,Size1Reader->GetOutput()->GetPixel(pixelIndex));
