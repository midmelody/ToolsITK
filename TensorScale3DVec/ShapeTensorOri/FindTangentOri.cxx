void FindTangentOri(float *DT, VOXEL *vecOri, VOXEL *tangentOri1, VOXEL *tangentOri2, bool smoothFlag)
{
  int i,j,k;//index
  ...
  int neighborSize; //neighborhood Size*2+1
  float maxDT=0;
  VOXEL grid[11][11]; //converted coordinate
  VOXEL orthoSampleVec[11][11];
  float sampleValue[121];
  for(i=0;i<pRow;i++)
    for(j=0;j<pCol;j++)
      for(k=0;k<pSli;k++)
  ...
  for(i=0;i<pRow;i++)
	...
	for(k=0;k<pSli;k++)
	  { 
	    ...
	    neighborSize = int(DT(i,j,k)/maxDT*3)+2;
	    ...
	    vnl_matrix<float> planePCA(121*2,2,0.0);
	      ...
	      for(b=-neighborSize;b<=neighborSize;b++)
		{
		  ...
		  planePCA(121+t,0) = -planePCA(t,0);
		  planePCA(121+t,1) = -planePCA(t,1);
		  t = t+1;
		}	    
