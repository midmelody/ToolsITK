void RasterScanEDT(int numberOfZeroCrossing, VOXEL *zeroCrossing, float *DT, VOXEL *NEP, int neighborCount) 
{
  ...
  for(t=0;t<numberOfZeroCrossing;t++)
	...
	for(b=0;b<=1;b++)
	  for(c=0;c<=1;c++)
		...
		{
		  if(DT(i+a,j+b,k+c)>((a-distX)*(a-distX)+(b-distY)*(b-distY)+(c-distZ)*(c-distZ))
		    {
		      DT(i+a,j+b,k+c)=(a-distX)*(a-distX)+(b-distY)*(b-distY)+(c-distZ)*(c-distZ);
  ...
  for(i=0;i<pRow;i++)
      ...
      for(k=0;k<pSli;k++) 
	{
	  ...
	  pixelIndex[1]=j;
	  pixelIndex[2]=k;
	  ZeroCrossingImage->SetPixel(pixelIndex,0);
	}
  for(t=0;t<numberOfZeroCrossing;t++)
    {
      edgeX = zeroCrossing[t].x;
      ...
      pixelIndex[1]=j;
      pixelIndex[2]=k;
      ZeroCrossingImage->SetPixel(pixelIndex,1);
    }
