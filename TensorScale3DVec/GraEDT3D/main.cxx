...
int main(int argc, char *argv[])
{
  ...
  if( argc < 4 )
    {
      ...
      std::cerr << argv[0] << " neighborhoodSize smoothFlagDT smoothFlagGraDT" << std::endl;
      return EXIT_FAILURE;
    }
  ...
  bool smoothFlagGraDT = atoi(argv[3]);
  //------------------Write result---------------------------------------
	...
	for(j=0;j<pCol;j++)
	  for(k=0;k<pSli;k++)
		...
		SmoothGraDTZ(i,j,k)=1.0*signf(SmoothGraDTZ(i,j,k));
	      ...
	      GradientDTXImage->SetPixel(pixelIndex,SmoothGraDTX(i,j,k)/norm);
	      GradientDTYImage->SetPixel(pixelIndex,SmoothGraDTY(i,j,k)/norm);
	      GradientDTZImage->SetPixel(pixelIndex,SmoothGraDTZ(i,j,k)/norm);
	    }
      free(SmoothGraDTX);
      free(SmoothGraDTY);
      free(SmoothGraDTZ);
