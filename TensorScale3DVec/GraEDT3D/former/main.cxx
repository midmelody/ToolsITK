...
int main(int argc, char *argv[])
{
  ...
  for(i=0;i<pRow;i++)
      ...
      for(k=0;k<pSli;k++) 
	{
	  ...
	  pixelIndex[1]=j;
	  pixelIndex[2]=k;
	  DTValue = ImageReader->GetOutput()->GetPixel(pixelIndex);
	  DTValue = DTValue * DTValue;
	  DTImage->SetPixel(pixelIndex,DTValue);
	}
  ...
  */
  //Get smoothed distance transform 
  float *DT;
  ...
  for(i=0;i<pRow;i++)
      ...
      for(k=0;k<pSli;k++) 
	{
	  ...
	  pixelIndex[1]=j;
	  pixelIndex[2]=k;
	  //DT(i,j,k) = DTImage->GetPixel(pixelIndex);
  ...
  for(i=0;i<pRow;i++)
      ...
      for(k=0;k<pSli;k++)
	{
	  ...
	  GradientDTXImage->SetPixel(pixelIndex,GradientDTX(i,j,k));
	  GradientDTYImage->SetPixel(pixelIndex,GradientDTY(i,j,k));
	  GradientDTZImage->SetPixel(pixelIndex,GradientDTZ(i,j,k));
	}
  /*
  ...
  GraMagType::Pointer graMagFilter = GraMagType::New();
  graMagFilter->SetInput( filterZ->GetOutput() );
  ImageWriterType::Pointer writerGradientMag = ImageWriterType::New();
  writerGradientMag->SetInput( graMagFilter->GetOutput() );
  writerGradientMag->SetFileName("temp/GradientMag.img");
  writerGradientMag->Update();
  */
  //----------------------Smooth the gradient field ----------------------------------------------
  std::cout<<"Smoothing the gradient of EDT: "<<std::endl;
  ...
  //Third,  add symmetric part to the matrix
  //Fourth, compute SVD for orientation result
  int pcaSize = (smoothSize*2+1)*(smoothSize*2+1)*(smoothSize*2+1)*2;
  vnl_matrix<float> graPairs(pcaSize,3,0.0);  
  float tempLength;
  for(i=0;i<pRow;i++)
    {
      std::cout<<"\r";
		  ...
		  if(((i+x)>=0)&&((i+x)<pRow)&&((j+y)>=0)&&((j+y)<pCol)&&((k+z)>=0)&&((k+z)<pSli))
		    {
		      ...
		      if(tempLength<0.15) tempLength=0;
		      ...
		      graPairs(pcaSize/2+t,0) = -GradientDTX(i+x,j+y,k+z)*tempLength;
		      graPairs(pcaSize/2+t,1) = -GradientDTY(i+x,j+y,k+z)*tempLength;
		      graPairs(pcaSize/2+t,2) = -GradientDTZ(i+x,j+y,k+z)*tempLength;
		      t = t+1;
		    }
	    //SVD
