...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  std::cout<<"Projection to 3 planes"<<std::endl;
  ...
  */
  ...
  std::cout<<std::endl<<"PCA filtering on orientation"<<std::endl;
  ...
  std::cout<<std::endl<<"Smoothing Anisotropy: "<<std::endl;
  //Assign color coding
  ...
  ImageType::Pointer aniImage = ImageType::New();
  aniImage->SetRegions( region );
  aniImage->SetSpacing( spacing );
  aniImage->Allocate();
  ...
  filterX->SetSigma( spaRow );
  filterY->SetSigma( spaCol );
  filterZ->SetSigma( spaSli );
  ...
  //Get anisotropy and apply filter to it
  for(i=0;i<pRow;i++)
    {
      std::cout<<"\r";
      std::cout<<int((i+1)*100/pRow)<<"%"<<std::flush;
	...
	for(k=0;k<pSli;k++)
	  {
	    ...
	    S = sqrt(1.0-minor*minor/(major*major));
	    pixelIndex[0]=i;
	    pixelIndex[1]=j;
	    pixelIndex[2]=k;
	    aniImage->SetPixel(pixelIndex,S);
	  }
    }
  filterX->SetInput( aniImage );
  filterY->SetInput( filterX->GetOutput() );
  filterZ->SetInput( filterY->GetOutput() );
  filterZ->Update();
  std::cout<<std::endl<<"Finish filter"<<std::endl;
  for(i=0;i<pRow;i++)
    {
      std::cout<<"\r";
      std::cout<<int((i+1)*100/pRow)<<"%"<<std::flush;
	...
	for(k=0;k<pSli;k++)
	  {
	    ...
	    pixelIndex[1]=j;
	    pixelIndex[2]=k;
	    S = filterZ->GetOutput()->GetPixel(pixelIndex);
	    major = tensorSmtXY(i,j,k).lengthMajor;
	    minor = tensorSmtXY(i,j,k).lengthMinor;
  ...
  aniImage->DisconnectPipeline();

  //---------------------XZ--------------------------
  std::cout<<std::endl<<"XZ:"<<std::endl;
  //Get anisotropy and apply filter to it
  for(i=0;i<pRow;i++)
    {
      std::cout<<"\r";
