...
int main(int argc, char *argv[])
{
  ...
  while(k<pSli)
	...
	for(j=0;j<pCol;j++)
	  {	
	    ...
	    ///////////////////////////
	    centerA1 = round(mag*0.6*sin(float(k)/sizeF*8*PI)+483);
	    centerB0 = 455;
	    if(((x-centerA1)*(x-centerA1)+(y-centerB0)*(y-centerB0))<9)
	      {
		a = round(x);
		b = round(y);
		c = k;
	...
	for(j=0;j<pCol;j++)
	  {	
	    ...
	    ////////////////////////////////////////
	    centerA0 = 70;
	    centerB0 = round(mag/2*sin(float(k)/sizeF*12*PI)+417);
	    if((float((x-centerA0)*(x-centerA0))/9+float((y-centerB0)*(y-centerB0))/49)<1)
	      {
		a = round(x);
		b = round(y);
		c = k;
	...
	for(j=0;j<pCol;j++)
	  {	
	    ...
	    if((length<15*15)||((length>25*25)&&(length<35*35))||((length>42*42)&&(length<50*50))||((length>55*55)&&(length<60*60))||((length>63*63)&&(length<66*66)))
	      {
		pixelIndex[0]=i;
		pixelIndex[1]=j;
		pixelIndex[2]=k;
	...
	for(j=0;j<pCol;j++)
	  {	
	    ...
	    if(fabs(x)>fabs(y))
	      ...
	      length = fabs(y);
	    if((length<15)||((length>25)&&(length<35))||((length>42)&&(length<50))||((length>55)&&(length<60))||((length>63)&&(length<66)))
	      {
		pixelIndex[0]=i;
		pixelIndex[1]=j;
		pixelIndex[2]=k;
  ...
  writer->SetFileName("phantomNew.img");
  writer->Update();
  return EXIT_SUCCESS;
}
