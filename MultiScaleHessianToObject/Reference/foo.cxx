...
int main( int argc, char *argv[] )
{
  ...
  std::cout<<multiScaleEnhancementFilter->GetScalesOutput()->GetRequestedRegion()<<std::endl;
  std::cout<<multiScaleEnhancementFilter->GetScalesOutput()->GetBufferedRegion()<<std::endl;
  //itk::ImageRegionConstIterator<OutputImageType> constit(multiScaleEnhancementFilter->GetScalesOutput(),  
  //                       multiScaleEnhancementFilter->GetScalesOutput()->GetRequestedRegion() );
        
  //for ( constit.GoToBegin(); !constit.IsAtEnd(); ++constit);
  //{
  //    std::cout<<constit.Get()<< " "; //this output produces "0" only
  //}
