...
float computeAffinityMap(AffinityType * affinityMap, float * inputImage, ImageType::SpacingType spacing, float sigma, float mean, bool darkObject)
{
  int i, j, k; 
  int a, b, c;
		  ...
		  if((iNeighbor>=0)&&(iNeighbor<pRow)&&(jNeighbor>=0)&&(jNeighbor<pCol)&&(kNeighbor>=0)&&(kNeighbor<pSli))
		    {
		      ...
		      affinity = exp(-(intensityQ-intensityP)*(intensityQ-intensityP)/(sigma*sigma));
		      if(darkObject)
			{
			  if(intensityQ<mean)
			    affinity = 1;
			  else	
			    affinity = affinity * exp(-(intensityQ-mean)*(intensityQ-mean)/(sigma*sigma));
			}
		      else
			{
			  if(intensityQ>mean)
