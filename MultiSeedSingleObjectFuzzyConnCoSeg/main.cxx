...
int main( int argc, char * argv[] )
{
  if( argc < 10 )
    {
      ...
      std::cerr << argv[0] << " seedFile outputImageFile CTImageFile PETImageFile CTSigmaHigh CTSigmaLow PETSigma CTMean PETMean" << std::endl;
      return EXIT_FAILURE;
    }
  ...
  VectorType sigmaHigh;
  sigmaHigh.CT = atoi(argv[5]);
  sigmaHigh.PET = atoi(argv[7]);

  VectorType sigmaLow;
  sigmaLow.CT = atoi(argv[6]);
  sigmaLow.PET = atoi(argv[7]);

  VectorType mean;
  mean.CT = atoi(argv[8]);
  mean.PET = atoi(argv[9]);
  ...
  kFOEMS( inputImage, spacing, sigmaHigh, sigmaLow, mean, seed, seedCount, fuzzyConImage);
  std::cout <<"finished"<<std::endl;
  std::cout<<std::endl;
  //Set output
