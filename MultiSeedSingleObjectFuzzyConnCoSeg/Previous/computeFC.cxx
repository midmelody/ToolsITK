...
void kFOEMS(VectorType * inputImage, ImageType::SpacingType spacing, VectorType sigma, VectorType mean, unsigned int * seedList, unsigned int seedCount, unsigned int *fuzzyConImage)
{ 
	...
	for(b=-1;b<=1;b++)
	  for(c=-1;c<=1;c++)
	      ...
	      if((iNeighbor>=0)&&(iNeighbor<pRow)&&(jNeighbor>=0)&&(jNeighbor<pCol)&&(kNeighbor>=0)&&(kNeighbor<pSli))
		{
		  ...
		  CTAffinity = exp(-(CTQ-CTP)*(CTQ-CTP)/(CTSigma*CTSigma));
		  //choose smaller one in calculation ( greater attenuation )
		  float CTSigmaHigh = 200;
		  if(CTP > CTQ)
		    CTPQ = CTP;
		  else
		    CTPQ = CTQ;
		  if(CTPQ<CTMean)
		    CTAffinity = CTAffinity * exp(-(CTPQ-CTMean)*(CTPQ-CTMean)/(CTSigma*CTSigma));
		  else
		  ...
		  affinity = affinity * sqrt(PETAffinity*CTAffinity);		  
